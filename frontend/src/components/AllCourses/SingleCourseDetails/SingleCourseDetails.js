import React from 'react';
import './SingleCourseDetails.css';
import Header from '../../Home/Header/Header';
import Footer from '../../Home/Footer/Footer';

const SingleCourseDetails = () => {
    return (
        <div className='s-body-d'>
            <Header />
            <div className="banner-course">
                <div className="banner-inner-course">
                    <div className="left_single">
                        <h1 className='primaryText'>The Complete 2024 Web Development Bootcamp</h1>
                        <div className="desc-c">
                            <p className='secondaryTextt'>Become a Full-Stack Web Developer with just ONE course. HTML, CSS, Javascript, Node, React, PostgreSQL, Web3 and DApps</p>
                        </div>
                        <div className="testimonials_rating">
                            <div className="no_of_students">
                                <p>12,222 - students</p>
                            </div>
                            <br />
                            <div className="ratings_course">
                                <span>&#9733;</span>
                                <span>&#9733;</span>
                                <span>&#9733;</span>
                                <span>&#9733;</span>
                                <span>&#9734;</span>
                            </div>
                            <div className="bottom_single">
                                <p className="secondaryText">
                                    Created by <u>Saklin Mustak</u>
                                </p>
                                <p className="secondaryText">Teacher's Email <u>saklin@123.gmail.com</u></p>
                                <p className="secondaryText">Last updated 3/2024</p>

                                <br />
                                <br />
                                <p className='c-desc'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Tempore quidem, <br /> rem nobis, molestias temporibus consequatur tempora repellat illum cum voluptate <br /> suscipit tenetur animi doloribus aliquid perferendis iusto iste pariatur. <br /> Aliquid exercitationem placeat repudiandae recusandae minima possimus <br /> deleniti obcaecati nihil nisi dolorum veritatis, <br /> nesciunt sequi ex nostrum necessitatibus saepe consectetur architecto.</p>
                            </div>
                        </div>
                    </div>

                    <div className="right_single">
                        <div className="right_single_inner">
                            <img className='img_course' src="https://play-lh.googleusercontent.com/rfWOJQVBHoAZ_B43v0ySFlLmJBLtksVGAxGaFRh2ex4nOmNQ86qzG4sYWV63IKrXlvI" alt="" />
                        </div>
                        <div className="mid_single">
                            <p className='mid-p'>
                                <span className='price-mid'>₹499</span>
                                <span><s>₹3,099</s></span>
                            </p>
                            <p className='mid-star'>
                                <span>&#9733;</span>
                                <span>&#9733;</span>
                                <span>&#9733;</span>
                                <span>&#9733;</span>
                                <span>&#9734;</span>
                            </p>
                            <span className='discount'>Discount84% off</span>
                        </div>
                        <div className="mid_single_button">
                            <button className='button w-100'>Buy Now</button>
                        </div>
                    </div>
                </div>
            </div>

            <div className="single_course_main">
                <div className="single_inner">
                    <h1 className='primaryText'>What you'll learn</h1>
                    <div className="ticks_single">
                        <div className='ul'>
                            <div className="li">
                                <li>Build 16 web development projects for your portfolio, ready to apply for junior developer jobs.</li>

                                <li>After the course you will be able to build ANY website you want.</li>

                                <li>Work as a freelance web developer.</li>

                                <li>Learn the latest technologies, including Javascript, React, Node and even Web3 development.</li>
                            </div>

                            <div className="li">
                                <li>Build fully-fledged websites and web apps for your startup or business.
                                </li>

                                <li>Master frontend development with React</li>

                                <li>Master backend development with Node</li>

                                <li>Learn professional developer best practices.</li>
                            </div>

                        </div>
                    </div>
                </div>
            </div>

            <div className=" single_course_main2">
                <div className=" single_inner2">
                    <h1 className='primaryText'>This course includes:</h1>
                    <div className="ticks_single ticks_single2">
                        <div className=' ul2'>
                            <div className=" li2">
                                <li>61 hours on-demand video</li>

                                <li>7 coding exercises</li>

                                <li>65 articles</li>
                            </div>

                            <div className=" li2">
                                <li>194 downloadable resources</li>

                                <li>Access on mobile and TV</li>

                                <li>Certificate of completion</li>
                            </div>

                        </div>
                    </div>
                </div>
            </div>


            <div className="cmmmm"><h1 className="primaryText">Comments/Feedbacks</h1></div>
            <div className="comments_single">

                <div className="single-comment">
                    <div className="comm_inner">
                        <img src="https://play-lh.googleusercontent.com/rfWOJQVBHoAZ_B43v0ySFlLmJBLtksVGAxGaFRh2ex4nOmNQ86qzG4sYWV63IKrXlvI" alt="" />
                        <p>Saklin Mustak</p>
                    </div>

                    <div className="quote">
                        <p>
                            <b>"</b> Lorem ipsum dolor sit amet consectetur adipisicing elit. Debitis repellendus aut corporis ullam incidunt obcaecati cumque. Laboriosam quo ratione, ex doloribus aliquam, ut nostrum consequatur aperiam enim voluptate illo, praesentium obcaecati repellat eveniet doloremque nulla rerum quis perspiciatis sequi! Nesciunt mollitia eos nostrum. Iusto deserunt nesciunt similique fugiat dolore, quos numquam vitae delectus corrupti, voluptatum cumque in nostrum ex incidunt? <b>"</b>
                        </p>
                    </div>
                </div>
                <div className="single-comment">
                    <div className="comm_inner">
                        <img src="https://play-lh.googleusercontent.com/rfWOJQVBHoAZ_B43v0ySFlLmJBLtksVGAxGaFRh2ex4nOmNQ86qzG4sYWV63IKrXlvI" alt="" />
                        <p>Saklin Mustak</p>
                    </div>

                    <div className="quote">
                        <p>
                            <b>"</b> Lorem ipsum dolor sit amet consectetur adipisicing elit. Debitis repellendus aut corporis ullam incidunt obcaecati cumque. Laboriosam quo ratione, ex doloribus aliquam, ut nostrum consequatur aperiam enim voluptate illo, praesentium obcaecati repellat eveniet doloremque nulla rerum quis perspiciatis sequi! Nesciunt mollitia eos nostrum. Iusto deserunt nesciunt similique fugiat dolore, quos numquam vitae delectus corrupti, voluptatum cumque in nostrum ex incidunt? <b>"</b>
                        </p>
                    </div>
                </div>
                <div className="single-comment">
                    <div className="comm_inner">
                        <img src="https://play-lh.googleusercontent.com/rfWOJQVBHoAZ_B43v0ySFlLmJBLtksVGAxGaFRh2ex4nOmNQ86qzG4sYWV63IKrXlvI" alt="" />
                        <p>Saklin Mustak</p>
                    </div>

                    <div className="quote">
                        <p>
                            <b>"</b> Lorem ipsum dolor sit amet consectetur adipisicing elit. Debitis repellendus aut corporis ullam incidunt obcaecati cumque. Laboriosam quo ratione, ex doloribus aliquam, ut nostrum consequatur aperiam enim voluptate illo, praesentium obcaecati repellat eveniet doloremque nulla rerum quis perspiciatis sequi! Nesciunt mollitia eos nostrum. Iusto deserunt nesciunt similique fugiat dolore, quos numquam vitae delectus corrupti, voluptatum cumque in nostrum ex incidunt? <b>"</b>
                        </p>
                    </div>
                </div>
                <div className="single-comment">
                    <div className="comm_inner">
                        <img src="https://play-lh.googleusercontent.com/rfWOJQVBHoAZ_B43v0ySFlLmJBLtksVGAxGaFRh2ex4nOmNQ86qzG4sYWV63IKrXlvI" alt="" />
                        <p>Saklin Mustak</p>
                    </div>

                    <div className="quote">
                        <p>
                            <b>"</b> Lorem ipsum dolor sit amet consectetur adipisicing elit. Debitis repellendus aut corporis ullam incidunt obcaecati cumque. Laboriosam quo ratione, ex doloribus aliquam, ut nostrum consequatur aperiam enim voluptate illo, praesentium obcaecati repellat eveniet doloremque nulla rerum quis perspiciatis sequi! Nesciunt mollitia eos nostrum. Iusto deserunt nesciunt similique fugiat dolore, quos numquam vitae delectus corrupti, voluptatum cumque in nostrum ex incidunt? <b>"</b>
                        </p>
                    </div>
                </div>
                <div className="single-comment">
                    <div className="comm_inner">
                        <img src="https://play-lh.googleusercontent.com/rfWOJQVBHoAZ_B43v0ySFlLmJBLtksVGAxGaFRh2ex4nOmNQ86qzG4sYWV63IKrXlvI" alt="" />
                        <p>Saklin Mustak</p>
                    </div>

                    <div className="quote">
                        <p>
                            <b>"</b> Lorem ipsum dolor sit amet consectetur adipisicing elit. Debitis repellendus aut corporis ullam incidunt obcaecati cumque. Laboriosam quo ratione, ex doloribus aliquam, ut nostrum consequatur aperiam enim voluptate illo, praesentium obcaecati repellat eveniet doloremque nulla rerum quis perspiciatis sequi! Nesciunt mollitia eos nostrum. Iusto deserunt nesciunt similique fugiat dolore, quos numquam vitae delectus corrupti, voluptatum cumque in nostrum ex incidunt? <b>"</b>
                        </p>
                    </div>
                </div>
                <div className="single-comment">
                    <div className="comm_inner">
                        <img src="https://play-lh.googleusercontent.com/rfWOJQVBHoAZ_B43v0ySFlLmJBLtksVGAxGaFRh2ex4nOmNQ86qzG4sYWV63IKrXlvI" alt="" />
                        <p>Saklin Mustak</p>
                    </div>

                    <div className="quote">
                        <p>
                            <b>"</b> Lorem ipsum dolor sit amet consectetur adipisicing elit. Debitis repellendus aut corporis ullam incidunt obcaecati cumque. Laboriosam quo ratione, ex doloribus aliquam, ut nostrum consequatur aperiam enim voluptate illo, praesentium obcaecati repellat eveniet doloremque nulla rerum quis perspiciatis sequi! Nesciunt mollitia eos nostrum. Iusto deserunt nesciunt similique fugiat dolore, quos numquam vitae delectus corrupti, voluptatum cumque in nostrum ex incidunt? <b>"</b>
                        </p>
                    </div>
                </div>
                <div className="single-comment">
                    <div className="comm_inner">
                        <img src="https://play-lh.googleusercontent.com/rfWOJQVBHoAZ_B43v0ySFlLmJBLtksVGAxGaFRh2ex4nOmNQ86qzG4sYWV63IKrXlvI" alt="" />
                        <p>Saklin Mustak</p>
                    </div>

                    <div className="quote">
                        <p>
                            <b>"</b> Lorem ipsum dolor sit amet consectetur adipisicing elit. Debitis repellendus aut corporis ullam incidunt obcaecati cumque. Laboriosam quo ratione, ex doloribus aliquam, ut nostrum consequatur aperiam enim voluptate illo, praesentium obcaecati repellat eveniet doloremque nulla rerum quis perspiciatis sequi! Nesciunt mollitia eos nostrum. Iusto deserunt nesciunt similique fugiat dolore, quos numquam vitae delectus corrupti, voluptatum cumque in nostrum ex incidunt? <b>"</b>
                        </p>
                    </div>
                </div>
                <div className="single-comment">
                    <div className="comm_inner">
                        <img src="https://play-lh.googleusercontent.com/rfWOJQVBHoAZ_B43v0ySFlLmJBLtksVGAxGaFRh2ex4nOmNQ86qzG4sYWV63IKrXlvI" alt="" />
                        <p>Saklin Mustak</p>
                    </div>

                    <div className="quote">
                        <p>
                            <b>"</b> Lorem ipsum dolor sit amet consectetur adipisicing elit. Debitis repellendus aut corporis ullam incidunt obcaecati cumque. Laboriosam quo ratione, ex doloribus aliquam, ut nostrum consequatur aperiam enim voluptate illo, praesentium obcaecati repellat eveniet doloremque nulla rerum quis perspiciatis sequi! Nesciunt mollitia eos nostrum. Iusto deserunt nesciunt similique fugiat dolore, quos numquam vitae delectus corrupti, voluptatum cumque in nostrum ex incidunt? <b>"</b>
                        </p>
                    </div>
                </div>
            </div>
            <div className="foo">
            <Footer />
            </div>
        </div>
    )
}

export default SingleCourseDetails;
